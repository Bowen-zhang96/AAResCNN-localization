...
class AttentionAugmentation2D(Layer):
    ...
    def rel_to_abs(self, x):
        ...
        B, Nh, L, = shape
        col_pad = tf.zeros(K.stack([B, Nh, L, 1]))
        x = K.concatenate([x, col_pad], axis=3)
        flat_x = K.reshape(x, [B, Nh, L * 2 * L])
        flat_pad = tf.zeros(K.stack([B, Nh, L - 1]))
...
if __name__ == '__main__':
    ...
    x = augmented_conv2d(ip, filters=32, kernel_size=(5, 5),
                         depth_k=8, depth_v=8,  # dk/v (0.2) * f_out (20) = 4
                         num_heads=8, relative_encodings=True)
